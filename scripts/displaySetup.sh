...
echo
    if [ "$#" -eq "0" ]; then
        ...
        xrandr --output $internal --mode 1920x1200 --pos -120x1050 --rotate normal --output $external --primary --mode 1680x1050 --pos 0x0 --rotate normal --output $extra --off
        echo
    elif [ "$#" -eq "1" ]; then
        if [ "$1" -le "5" ]; then
            case $1 in
                0)
                    ...
                    xrandr --output $internal --primary --mode 1920x1200 --pos 0x0 --rotate normal --output $external --off --output $extra --off
                    echo;;
				1|a|A)
                    echo "External Monitor: Haier TV"
                    xrandr --output $internal --mode 1920x1200 --pos 0x1080 --rotate normal --output $external --primary --mode 1920x1080 --pos 0x0 --rotate normal --output $extra --off
                    echo;;
                *)
